...
def daily_sync():
    try:
        ...
        # משיכת פריטים קיימים למניעת כפילויות
        query_existing = '''
        query {
          boards(ids: %s) {
        ...
        for row in all_rows_content[1:]:
            ...
            if f"{user_name}_{raw_date}" in existing_records: continue
        ...
        return f"Sync complete! Added {sent_count} items.", 200
